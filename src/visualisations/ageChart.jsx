...
const AgeChart = ({xlabel, ylabel}) => {
    let element = useRef(null)

    let stats = useAgeStats()
    ...
    useEffect(() => {
        ...
        let ages = stats.map(s => s.age)

        let x = d3.scaleLinear()
            .domain([d3.min(ages) - 10, d3.max(ages) + 10])
            .range([0, width])
        ...
        Svg.selectAll("rect")
            ...
            .append("rect")
              .attr("x", 1)
              .attr("transform", function(d) { return "translate(" + x(d.age - 1/2) + "," + y(d.users) + ")"; })
              .attr("width", function(d) { return x(d.age) - x(d.age - 1) - 2 })
